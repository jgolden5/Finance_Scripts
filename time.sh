...
commute_and_work() {
  ...
  local total_hours_worked_without_commute=0
  ...
  for n in $(seq 1 $days_per_week); do
    ...
    total_hours_worked_without_commute="$(echo "scale=2; $total_hours_worked_without_commute + $hours_on_day_n" | bc)"
  ...
  echo "Total hours work (without commute) = $total_hours_worked_without_commute"
